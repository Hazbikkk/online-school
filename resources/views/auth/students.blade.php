...
<body>
    ...
    <form method="POST" action = "{{ route('register.students.store') }}">
        ...
        @enderror
        <h1 class="block w-full max-w-md mx-auto">Имя:</h1> <input class="block w-full max-w-md mx-auto p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" id="name" name="name" type="text" placeholder="Ваше имя которое вводили при регистрации"><br>
        @error('code')
        <span style="color: red;">{{ $message }}</span>
        @enderror
